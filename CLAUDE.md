...
- `bot/사건봇.js` + `bot/사건봇로더.js` — "사건" 방 전용 재련 미니게임 (제련봇과 별개, 파괴/수리 없음).
  로더 명령은 `/사건업데이트`, `/사건로더`
- `bot/오토봇.js` + `bot/오토봇로더.js` + `bot/오토봇데이터.json` — "오토2프프"·"오토2"·"[오차율 계산봇]"·"공백기 근무표" 방 자동응답 봇.
  **읽기 전용**: 봇에 등록/삭제 명령이 없고, 자동응답 내용은 `오토봇데이터.json`을 고쳐서 관리한다(=Claude에게 요청).
  본체가 깃헙에서 그 JSON을 받아 쓰고 폰에 캐시하므로 네트워크가 끊겨도 마지막 내용으로 동작한다.
  갱신은 앱 시작 시 / 30분마다 / `/오토업데이트` 즉시. 방 명령은 `/리스트`(누구나), `/오토`(진단),
  로더 명령은 `/오토업데이트`, `/오토로더`. 데이터 구조는 `{"_공통":{트리거:내용}, "방이름":{...}}` 이고
  같은 트리거면 방별이 공통을 덮어쓴다. 매칭은 메시지 전체 일치(앞뒤 공백만 제거)이나,
  **트리거 이름이 `*` 로 시작하면 그 낱말이 대화에 섞여 있기만 해도 응답**한다(예: `"*질문"`).
  일상 대화에서 터지는 말이라 방마다 20분(`CONTAIN_COOL_MIN`) 쿨다운을 두고, 전체 일치가 항상 우선한다.
  쿨다운은 트리거 이름이 아니라 **내보낼 내용**으로 묶으므로, `*질문`·`*궁금` 처럼 같은 안내를 가리키는
  낱말이 여럿이어도 한 방에 20분에 한 번만 나간다.
  `/리스트` 에는 `질문 (말 속에 있어도)` 형태로 보인다
  **대화 기록**도 겸한다 — 오토2·오토2프프·공백기 근무표의 메시지를 방마다 3000개까지 보관하고,
  "공백기 근무표" 방에서 `/삭제내역`(이 방)·`/삭제내역1`(오토2)·`/삭제내역2`(오토2프프)로 되짚어본다.
  카톡은 삭제를 봇에게 알려주지 않으므로 "어느 게 삭제됐는지"는 알 수 없고, 시각으로 대조하는 용도다.
  ※ **메시지 처리 흐름에서는 네트워크·긴 대기를 절대 하지 않는다.** 그러면 봇 전체가 멈추고,
  오래 멈추면 안드로이드가 앱을 정지시킨다 (2026-09-07 실제로 겪음). 데이터 갱신·파일 저장은 백그라운드로 돌린다
- `bot/카페봇.js` + `bot/카페봇로더.js` — 네이버 카페 새글 알림 **전용** 봇 (오토2프프·오토2).
  원래 오토봇에 있었으나, 네트워크·타이머 문제가 자동응답까지 끌고 내려가서 **2026-09-07 분리**했다.
  봇 앱은 스크립트마다 따로 돌리므로 서로 영향이 없다. 로더 명령은 `/카페업데이트`, `/카페로더`.
  방 명령은 `/카페`(누구나), `/새글테스트`·`/카페확인`·`/깃토큰`·`/발송테스트`·`/기능조사`(관리자).
  **폰에서 네이버를 직접 부르면 세션이 거부된다** — 쿠키 8개 전부·User-Agent·공인 IP·브라우저 헤더를
  모두 똑같이 맞춰도 `로그인하지 않았습니다`가 나오는데, **같은 쿠키를 깃헙 Actions 에서 쓰면 성공한다**.
  그래서 경로가 이렇다:
  `.github/workflows/cafe-watch.yml`(쿠키는 `NAVER_COOKIE` 시크릿, 쓰기는 `CAFE_REPO_TOKEN`)
  → 비공개 저장소 `limbj1218-cyber/cafe-watch` 의 `latest.json` → 카페봇이 깃헙 토큰으로 읽어 새 글만 발송.
  워크플로우를 **공개** 저장소에 둔 이유는 비공개 저장소 Actions 가 월 2000분 한도라서다.
  대신 **글 제목은 절대 로그에 출력하지 않는다** (공개 저장소는 실행 로그도 공개).
  깃헙 **예약 실행은 몇 시간씩 건너뛰므로** 믿을 수 없다 — 봇이 3분마다 워크플로우를 직접 깨우고
  (토큰에 chatlog 의 Actions 쓰기 권한 필요), 백그라운드 스레드일 때만 12/24/39초 기다렸다 읽는다.
...
> 둘 다 반응하므로, 방 목록(ROOMS)이 겹치지 않도록 유지할 것.
> `/리스트` 는 단톡봇과 오토봇이 공유하는 이름이다 (현재 단톡봇=임병진, 오토봇=오토2프프·오토2 로 방이 갈려 있어 충돌 없음).
- `faq/index.html` — 오토워커 2기 사담방(오토2) 자주 묻는 질문 페이지.
  2026-08-01~09-11 대화 53,442건에서 질문과 조교 답변을 추려 7개 주제 64문항으로 정리했다.
  주제 전환은 **라디오+라벨(CSS)** 로 구현 — 이 페이지를 정적 스냅샷으로 띄우는 뷰어에서도 동작해야 하므로
  자바스크립트에 의존하는 방식(앵커 스크롤·`:target`)으로 되돌리지 말 것. 검색창만 JS 이며 없으면 숨는다.
  `https://limbj1218-cyber.github.io/chatlog/faq/` 로 서비스되고, 오토봇의 `*질문` 트리거가 이 주소를 안내한다
- `index.html` — 루트 페이지. **의도적으로 비워 둔 빈 화면**이다 (2026-09-11, 사용자 요청).
  예전 단일 정리 페이지는 git 이력에 있다. 방별 페이지(`rooms/…`)와 `faq/` 는 하위 경로라 영향받지 않으며,
  단톡봇의 `PAGE_BASE` 도 `rooms/방이름/` 앞에만 붙으므로 그대로 동작한다. 여기에 내용을 되살리지 말 것
...
- 블록은 모든 페이지에 들어 있지만 JS의 `ROOM_CREW` 설정에 있는 방에서만 화면에 나타난다
  ...
  라그X하늘(폴더는 거북) = 카드줍꼬(궁수, 화살 발사). `ROOM_CREW` 키는 페이지 표시 이름 기준.
  다른 방에 보이게 하려면 `ROOM_CREW`에 방 이름과 멤버 구성만 추가.

### 방별 표시 이름 (별칭)

- **거북 방**: 파일 경로는 `rooms/거북/`이지만 페이지 표시 이름(title, og:title, hero h1)은 **"라그X하늘"**이다.
...
- 일시정지: `PAUSED_ROOMS` 배열에 방 이름을 넣으면 폰의 방 목록(로더 MY_ROOMS)과 무관하게 그 방을 완전히 무시
  (로그·명령·자동응답·자동벽타기 전부 중단). 현재 "거북"(라그X하늘) 일시정지 중.
...
- 벽타기 전체 스위치: `WALLCLIMB_ENABLED` (현재 `false`) — 꺼지면 위 벽타기 관련 기능(수동/자동 업로드,
  정시 공지, 전날 자동 마감)이 모든 방에서 전부 중단됨. `/방이름` 진단에서 상태 확인 가능.
