...
- `bot/오토봇.js` + `bot/오토봇로더.js` + `bot/오토봇데이터.json` — "오토2프프"·"오토2"·"[오차율 계산봇]" 방 자동응답 봇.
  ...
  **폰에서 네이버를 직접 부르면 세션이 거부된다** — 쿠키 8개 전부·User-Agent·공인 IP·브라우저 헤더를
  ...
  → 비공개 저장소 `limbj1218-cyber/cafe-watch` 의 `latest.json` → 오토봇이 깃헙 토큰으로 읽어 새 글만 발송.
  워크플로우를 **공개** 저장소에 둔 이유는 비공개 저장소 Actions 가 월 2000분 한도라서다.
  대신 **글 제목은 절대 로그에 출력하지 않는다** (공개 저장소는 실행 로그도 공개).
  폰에는 깃헙 토큰만 둔다 — `/깃토큰` (관리자, `/깃토큰 삭제` 로 제거).
  깃헙 **예약 실행은 몇 시간씩 건너뛰므로** 믿을 수 없다. 그래서 봇이 5분마다 확인할 때
  워크플로우를 `workflow_dispatch` 로 직접 깨운다(그래서 토큰에 chatlog 의 Actions 쓰기 권한이 필요).
  한 주기 안에서 **깨우기 → 갱신 확인(12/24/39초) → 읽기**를 다 하므로 새 글은 보통 1~4분 안에 도착한다.
  `/쿠키`·`/UA` 는 폰에서 직접 부르던 시절의 잔재로 지금 경로에서는 쓰이지 않는다

> 네 봇(단톡봇·제련봇·사건봇·오토봇)은 서로 독립이며 각자 로더로 설치한다. 명령어가 겹치지 않게 로더 명령을 구분해 두었으나,
