...
> **로더의 철칙 (2026-09-15, 봇이 반복해서 죽어서 알아냄)**: 로더는 깃헙에서 본체 코드를 받아 실행하는데,
> 받아오기가 실패하면 본체가 없는 채로 남고 그 뒤로 **메시지가 올 때마다 메시지 흐름에서 네트워크를 다시 시도하며
> 최대 20초씩 멈춘다.** 그게 쌓이면 안드로이드가 앱을 정지시킨다. 그래서 오토봇·카페봇 로더는
> ① 성공한 본체 코드를 폰에 저장해 두고(`오토봇본체캐시.js`·`카페봇본체캐시.js`), 깃헙이 안 되면 그걸로 버티며
> ② 메시지 흐름에서는 **캐시만** 즉시 읽고 깃헙 받아오기는 백그라운드로 넘긴다.
> `loadRemote(useNetwork)` 의 인자를 없애거나 메시지 흐름에서 `loadRemote(true)` 를 부르지 말 것.
> `/오토로더`·`/카페로더` 의 "코드 출처" 가 계속 `폰에 저장된 코드` 면 깃헙 쪽을 봐야 한다는 신호다
...
- `bot/오토봇.js` + `bot/오토봇로더.js` + `bot/오토봇데이터.json` — "오토2프프"·"오토2"·"[오차율 계산봇]"·"공백기 근무표" 방 자동응답 봇.
  **읽기 전용**: 봇에 등록/삭제 명령이 없고, 자동응답 내용은 `오토봇데이터.json`을 고쳐서 관리한다(=Claude에게 요청).
  ...
  같은 트리거면 방별이 공통을 덮어쓴다. **값이 문자열이면 메시지 1개, 목록(배열)이면 항목마다 나눠서 여러 개**를 보낸다
  (예: `"야담플로우": ["링크", "[메뉴얼]\n링크"]` → 2개 발송). 매칭은 메시지 전체 일치(앞뒤 공백만 제거)이나,
