...
- `bot/오토봇.js` + `bot/오토봇로더.js` + `bot/오토봇데이터.json` — "오토2프프"·"오토2"·"[오차율 계산봇]" 방 자동응답 봇.
  ...
  **네이버 카페 새글 알림**도 겸한다 — `CAFE` 설정의 카페를 10분마다 확인해 새 글을 제목+링크로 방에 발송
  (`/카페` 로 상태 확인). 타이머·발송은 단톡봇과 같은 3단 폴백. 첫 가동 땐 알리지 않고 기준 글번호만 잡는다.
  비공개 카페는 네이버 로그인 쿠키가 필요하며, 쿠키는 **깃헙에 절대 올리지 않는다**.
  넣는 방법은 ① 방에서 `/쿠키` (관리자만) — NID_AUT → NID_SES 를 한 번에 하나씩 물어보고 받는다
  (`/쿠키 NID_AUT=..; NID_SES=..` 한 줄 방식도 됨, 중간에 `/취소`). 폰 파일에 저장 후 즉시 검증
  ② 로더의 `MY_COOKIE` ③ 폰의 `<캐시폴더>/naver_cookie.txt`. `/쿠키삭제`로 제거.
  쿠키가 만료되면 6시간에 한 번 방에 알린다
