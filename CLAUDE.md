...
- 블록은 모든 페이지에 들어 있지만 JS의 `MASCOT_ROOMS` 목록(현재 `["금광2"]`)에 있는 방에서만
  화면에 나타난다 (왼쪽 아래 고정 정렬). 다른 방에 보이게 하려면 그 목록에 방 이름만 추가.
...
- 링크 공지: 09:00/15:00/21:00에 새 내용이 있으면 봇이 방에 페이지 링크를 공지 (Api.replyRoom)
