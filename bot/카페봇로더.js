/**
 * ═══════════════════════════════════════════════════════════
 *  카페봇 로더 — 이 파일만 봇 앱(메신저봇R 등)에 붙여넣으면 됩니다.
 *
 *  동작: GitHub에 있는 본체 코드(bot/카페봇.js)를 읽어와 실행합니다.
 *  - 방 목록은 이 로더(폰)에만 두어도 되고, 비워두면 본체 기본값을 씁니다
 *  - 코드가 갱신되면 방에서 /카페업데이트 (관리자만) 또는 재컴파일
 *    → 본체를 다시 불러오면서 카페 알림만 담당합니다 (자동응답은 오토봇)
 *
...
// 봇이 "반응"할 방 (명령어). 새 글 알림을 "보낼" 방은 본체의 CAFE.rooms 이다.
var MY_ROOMS = [
    "오토2프프",
    "오토2",
    "[오차율 계산봇]"
];
// /카페업데이트·/깃토큰 을 쓸 수 있는 사람 (대화명에 이 문자열이 포함되면 허용)
var SUPER_ADMINS = ["후파", "임병진", "[오차율 계산봇]"];

// 깃헙 토큰 (비공개 저장소 읽기 + chatlog 의 Actions 실행 권한).
// 보통은 여기 두지 않고 카톡에서 /깃토큰 <토큰> 으로 넣는 게 편하다 (폰 파일에 저장된다).
// ★ 이 값은 폰에만 두고 절대 깃헙에 올리지 않는다.
var MY_TOKEN = "";
// ────────────────────────────────────────────────────────────

var SRC_URL = "https://raw.githubusercontent.com/limbj1218-cyber/chatlog/main/bot/" +
    encodeURIComponent("카페봇.js");
...
function fetchText(url) {
    ...
    conn.setRequestProperty("User-Agent", "cafebot-loader");
...
function evalCode(src) {
    ...
    try {
        ...
        try { return cx.evaluateString(GLOBAL_SCOPE, src, "cafebot", 1, null); }
...
function cleanOldModules(dir, keep) {
    try {
        ...
        for (var i = 0; i < files.length; i++) {
            ...
            if (n.indexOf("cafebot_core_") === 0 && n !== keep) files[i]["delete"]();
        }
    } catch (e) {}
}
 ...
 * (파일명에 시각을 넣어 require 캐시를 피하므로 /카페업데이트 가 매번 최신을 반영)
 */
function loadViaRequire(code) {
    ...
    var fname = "cafebot_core_" + new Date().getTime();
    var body = "module.exports = function (__ROOMS__, __TOKEN__, __ADMINS__) {\n" + code +
        "\nif (__ROOMS__ && __ROOMS__.length > 0) ROOMS = __ROOMS__;" +
        "\nif (__ADMINS__ && __ADMINS__.length > 0) ADMINS = __ADMINS__;" +
        "\nif (__TOKEN__) GH_TOKEN = __TOKEN__;" +
        "\nreturn response;\n};";
...
function loadRemote() {
    ...
    // ① 동적 평가 허용 앱
    var wrapped = "(function (__ROOMS__, __TOKEN__, __ADMINS__) {\n" + code +
        "\nif (__ROOMS__ && __ROOMS__.length > 0) ROOMS = __ROOMS__;" +
        "\nif (__ADMINS__ && __ADMINS__.length > 0) ADMINS = __ADMINS__;" +
        "\nif (__TOKEN__) GH_TOKEN = __TOKEN__;" +
        "\nreturn response;\n})";
...
function handle(room, msg, sender, isGroupChat, replier) {
    ...
    try {
        ...
        if (shouldReportError(text)) {
            try { replier.reply("⚠️ 카페봇 로더 오류: " + e); } catch (e2) {}
        }
    }
}
