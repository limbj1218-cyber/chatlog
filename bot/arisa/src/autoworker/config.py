...
#: 자동응답이 동작하는 방 — 아래는 **설정에서 쓰는 짧은 이름**이다.
#: 실제 카톡 방 이름은 ROOM_ALIASES 로 이어 붙인다.
ROOMS = ["오토2프프", "오토2"]
EXTRA_ROOMS = ["[오차율 계산봇]", "공백기 근무표"]
ALL_ROOMS = ROOMS + EXTRA_ROOMS

#: channel_id → 설정에서 쓰는 짧은 이름. **이게 가장 확실한 방법이다.**
#:
#: 카톡 방 이름은 언제든 바뀌지만 channel_id 는 안 바뀐다. 그리고 여기에 적어두면
#: 봇이 켜지자마자 먼저 말을 걸 수 있다 (이름만 알면 그 방에서 메시지가
#: 한 번 올라올 때까지 기다려야 한다).
#:
#: 새 방의 id 는 그 방에서 ``/방정보`` 를 치면 나온다.
ROOM_IDS = {
    18490098569406776: "오토2",
    18490098487826738: "오토2프프",
}

#: 실제 카톡 방 이름 → 짧은 이름 (id 를 아직 모르는 방을 위한 보조 수단)
#:
#: 짧은 이름은 ``bot/오토봇데이터.json`` 의 키이자 메신저봇R 판과 공유하는 값이라
#: 그대로 두고, 카톡 쪽 이름만 여기에 이어 붙인다.
ROOM_ALIASES = {
    "(사담방) 오토워커 2기 [개발남노씨]": "오토2",
    "(프리미엄반) 오토워커 2기 [개발남노씨]": "오토2프프",
}


def canonical_room(name: str) -> str:
    """카톡 방 이름을 설정에서 쓰는 짧은 이름으로. 모르는 방은 그대로 둔다."""
    return ROOM_ALIASES.get(name, name)


def resolve_room(channel_id: int, name: str) -> str:
    """id 를 먼저 보고, 없으면 이름으로. 둘 다 모르면 받은 이름 그대로."""
    known = ROOM_IDS.get(channel_id)
    if known:
        return known
    return canonical_room(name)
