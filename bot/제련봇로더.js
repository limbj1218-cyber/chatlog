/**
 * ═══════════════════════════════════════════════════════════
 *  제련봇 로더 — 이 파일만 봇 앱(메신저봇R 등)에 붙여넣으면 됩니다.
 *
 *  동작: GitHub에 있는 본체 코드(bot/제련봇.js)를 읽어와 실행합니다.
 *  - 방 목록과 관리자 목록은 이 로더(폰)에만 두어도 되고, 비워두면 본체 기본값을 씁니다
 *  - 본체가 갱신되면 방에서 /제련업데이트 (관리자만) 또는 재컴파일
 *
...
function fetchText(url) {
    ...
    conn.setRequestProperty("User-Agent", "jeryeon-loader");
...
function evalCode(src) {
    ...
    try {
        ...
        try { return cx.evaluateString(GLOBAL_SCOPE, src, "jeryeon", 1, null); }
...
function cleanOldModules(dir, keep) {
    try {
        ...
        for (var i = 0; i < files.length; i++) {
            ...
            if (n.indexOf("jeryeon_core_") === 0 && n !== keep) files[i]["delete"]();
        }
    } catch (e) {}
}
...
function loadViaRequire(code) {
    ...
    var fname = "jeryeon_core_" + new Date().getTime();
...
function handle(room, msg, sender, isGroupChat, replier) {
    ...
    try {
        ...
        if (text === "/제련업데이트") {
            ...
            loadRemote();
            replier.reply("🔄 깃헙에서 최신 제련봇 코드를 불러왔어요! (" + loadMethod + ")");
            return;
        }
        if (text === "/제련로더") {
            replier.reply("🧩 제련봇 로더 상태\n" +
                "API 방식: " + apiMode + "\n" +
        ...
        if (shouldReportError(text)) {
            try { replier.reply("⚠️ 제련봇 로더 오류: " + e); } catch (e2) {}
        }
    }
}
